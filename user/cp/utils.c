...
#define YESNO "(y/n [n]) "
        ...
        if (fflag) {
            ...
            if (!lflag && !sflag) {
                to_fd = open(to.p_path, O_WRONLY | O_TRUNC | O_CREAT, 0x180);
            }
        } else if (!lflag && !sflag) {
    ...
    if (!lflag && !sflag) {
...
#endif
        {
            if (buf == NULL) {
                 ...
                 */
                long maxphys = sysconf(_SC_PHYS_PAGES);
                if (maxphys > PHYSPAGES_THRESHOLD)
...
int setfile(struct stat *fs, int fd)
{
    ...
    if (fdval ? fstat(fd, &ts) : stat(to.p_path, &ts))
        gotstat = 0;
    else {
        gotstat = 1;
...
*/
    if (!gotstat || fs->st_mode != ts.st_mode)
        if (fdval ? fchmod(fd, fs->st_mode) : chmod(to.p_path, fs->st_mode)) {
