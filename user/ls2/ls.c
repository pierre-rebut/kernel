...
#else
        warn("color support not compiled in");
#endif /*COLORLS*/
    }
...
static void display(const FTSENT *p, FTSENT *list, int options)
{
    ...
    for (cur = list, entries = 0; cur; cur = cur->fts_link) {
        ...
        if (needstats) {
            sp = cur->fts_statp;
            if (sp->st_blocks > (u32) maxblock)
                maxblock = (long) sp->st_blocks;
            if (sp->st_ino > maxinode)
                maxinode = sp->st_ino;
            if (sp->st_nlink > maxnlink)
                maxnlink = sp->st_nlink;
            if (sp->st_size > (u32) maxsize)
                maxsize = (long) sp->st_size;
