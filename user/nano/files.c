...
int open_file(const char *filename, bool newfie, bool quiet, FILE **f)
{
	...
	if (fd == -1)
		...
		*f = fdopen(fd); // todo check fdopen(fd, "rb")
		...
		kind_of_writing_type method, bool fullbuffer)
{
	...
	if (ISSET(BACKUP_FILE) && !tmp && realexists && openfile->current_stat &&
				...
				openfile->current_stat->st_mtim == st.st_mtim)) {
		static time_t filetime[2];
		char *backupname;
		int backup_cflags, backup_fd;
		FILE *backup_file = NULL;

		/* Save the original file's access and modification times. */
		filetime[0] = openfile->current_stat->st_atim;
		filetime[1] = openfile->current_stat->st_mtim;

		if (f_open == NULL) {
			/* Open the original file to copy to the backup. */
			f = fopen(realname, O_RDONLY, 0);
	...
	if (f_open == NULL) {
		...
		f = fdopen(fd); // todo (method == APPEND) ? "ab" : "wb"
	...
	if (method == PREPEND) {
		...
		if (fd_source != -1) {
			f_source = fdopen(fd_source); // todo "rb"
			if (f_source == NULL)
				close(fd_source);
		}
