...
static int heap_grow(size_t size, u8 *heapEnd, int continuous) {
...
void *kmalloc(size_t size, u32 alignment) {
    ...
    int continuous = alignment & HEAP_CONTINUOUS;
        ...
        if (!regions[i].reserved && (regions[i].size >= size + additionalSize) &&
            ...
            if (continuous) {
                ...
                for (void *virt1 = (void *) alignDown((u32) alignedAddress, PAGESIZE);
                    ...
                    u32 phys1 = getPhysAddr(virt1);
                    u32 phys2 = getPhysAddr(virt1 + PAGESIZE);
...
void heap_logRegions(void) {
    ...
    printf("\n\nregionMaxCount: %u, regionCount: %u, firstFreeRegion: %u\n", regionMaxCount, regionCount,
           firstFreeRegion);
    printf("\n---------------- HEAP REGIONS ----------------\n");
    printf("#\taddress\t\tsize\t\tnumber\tcomment");
